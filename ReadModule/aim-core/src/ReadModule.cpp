  ...
  cliParam(0)
{
  ...
  portInputIn.sock = new zmq::socket_t(*context, ZMQ_REP);
  portInputIn.ready = true;
  zmq_sockets.push_back(&portInputIn);
}

ReadModule::~ReadModule() {
  delete cliParam;
  delete portInputIn.sock;
}
...
void ReadModule::Init(std::string & name) {
  ...
  
  {
    ...
    zmqPortName = "/readmodule" + cliParam->module_id + "/input";
    portInputIn.name = zmqPortName;
    zmqPortName = "/resolve" + portInputIn.name;
    pns_record record;
    ...
    std::cout << "Bind to socket " << zmqPortName << std::endl;
    portInputIn.sock->bind(zmqPortName.c_str());
  }
}
...
void ReadModule::Resolve(pns_record & record) {
...
void ReadModule::SendAck(zmq::socket_t *s, bool state) {
...
char* ReadModule::GetReply(zmq::socket_t *s, bool & state, bool blocking, int & reply_size) {
  if (s == NULL)
    return NULL;
  zmq::message_t reply;
  char* result = NULL;
...
void ReadModule::SendRequest(zmq::socket_t *s, bool & state, bool blocking, std::string str) {
  if (state) {
...
zmq::socket_t* ReadModule::GetSocket(std::string name) {
