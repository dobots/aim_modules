  ...
  cliParam(0)
{
  ...
  portOutputOut.sock = new zmq::socket_t(*context, ZMQ_REQ);
  portOutputOut.ready = true;
  zmq_sockets.push_back(&portOutputOut);
}

WriteModule::~WriteModule() {
  delete cliParam;
  delete portOutputOut.sock;
}
...
void WriteModule::Init(std::string & name) {
  ...
  
  {
    ...
    zmqPortName = "/writemodule" + cliParam->module_id + "/output";
    portOutputOut.name = zmqPortName;
    zmqPortName = "/resolve" + portOutputOut.name;
    pns_record record;
...
void WriteModule::Resolve(pns_record & record) {
...
void WriteModule::SendAck(zmq::socket_t *s, bool state) {
  if (debug) std::cout << "Send ACK" << std::endl;
  SendRequest(s, state, true, "ACK");
}

bool WriteModule::ReceiveAck(zmq::socket_t *s, bool & state, bool blocking) {
  int reply_size = 0;
...
char* WriteModule::GetReply(zmq::socket_t *s, bool & state, bool blocking, int & reply_size) {
  if (s == NULL)
    return NULL;
  zmq::message_t reply;
  char* result = NULL;
...
void WriteModule::SendRequest(zmq::socket_t *s, bool & state, bool blocking, std::string str) {
  if (state) {
...
void WriteModule::HandleCommand() {
  int reply_size = -1;
  bool state = false;
...
void WriteModule::Connect(std::string source, std::string target) {
...
zmq::socket_t* WriteModule::GetSocket(std::string name) {
...
bool WriteModule::writeOutput(const int output) {
  ...
  pthread_mutex_lock(&cmdMutex);
  bool state = portOutputOut.ready;
  SendRequest(portOutputOut.sock, state, true, ss.str());
  if (state) portOutputOut.ready = false;
  if (!portOutputOut.ready) {
    bool ack_state = true;
    ReceiveAck(portOutputOut.sock, ack_state, true);
    if (ack_state) {
      portOutputOut.ready = true;
      pthread_mutex_unlock(&cmdMutex);
      return true;
